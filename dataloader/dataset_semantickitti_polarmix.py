...
from dataloader.polarmix import polarmix
...
class cylinder_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Flags pour utiliser la fonction Polarmix ou la fonction déjà implémentée
        use_polarmix = True
        use_alternative = False

        if use_polarmix:
            index2 = np.random.randint(len(self.point_cloud_dataset))
            data2 = self.point_cloud_dataset[index2]
            if len(data2) == 2:
                xyz2, labels2 = data
                sig2 = None
            elif len(data2) == 3:
                xyz2, labels2, sig2 = data
                if len(sig2.shape) == 2: sig2 = np.squeeze(sig2)
            else:
                raise Exception('Return invalid data tuple for second data')
            # Polarmix
            alpha_pm = (np.random.random() - 1) * np.pi 
            beta_pm = alpha_pm + np.pi 
            instance_classes = list(range(len(REGISTERED_DATASET_CLASSES)))
            xyz, labels, sig = polarmix(xyz, labels, xyz2, labels2, instance_classes, Omega, alpha_pm, beta_pm, sig, sig2)
